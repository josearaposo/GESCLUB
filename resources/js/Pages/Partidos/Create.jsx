...
export default function Create({ divisiones, equipos, jugadores }) {
    ...
    const toggleSeleccion = (id, rol) => {
        ...
        const seleccionActual = Array.isArray(data[rol]) ? data[rol] : [];
        const seleccionOtroRol = Array.isArray(data[otroRol]) ? data[otroRol] : [];

        if (seleccionActual.includes(id)) {
            // Quitar del rol actual
            setData({
                ...data,
                [rol]: seleccionActual.filter((x) => x !== id),
            });
        } else {
            // Añadir al rol actual y quitar del otro
            setData({
                ...data,
                [rol]: [...seleccionActual, id],
                [otroRol]: seleccionOtroRol.filter((x) => x !== id),
            });
        }
    };
                    ...
                    {jugadores.map((j) => (
                        <label key={j.id} className="flex items-center gap-2">
                            <input
                                ...
                                disabled={
                                    !data.titulares.includes(j.id) &&
                                    data.titulares.length >= 11
                                }
                            />
                            {j.nombre}
                        </label>
